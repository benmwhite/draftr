...
**draftr** is a package for computing the projected fantasy points over replacement based on user-supplied projection data. As our example we will use a subset of NBA players from the January 30, 2018 NBA main slate on Draft.com. This example data is included with the package as the data frame **proj**.
...
```{r}
my_draft <- get_porp(proj, teams = 3)
head(my_draft$df)
my_draft$chart
```
